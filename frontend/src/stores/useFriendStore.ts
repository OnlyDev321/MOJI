...
export const useFriendStore = create<FriendState>((set, get) => ({
  ...
  searchByUsername: async (username) => {
    try {
      ...
      console.error("ERROR when searchByUsername", error);
      return null;
    } finally {
      set({ loading: false });
    }
  ...
  addFriend: async (to, message) => {
    try {
      ...
      throw error.response?.data?.message || "ERROR when addFriend, try do it";
    } finally {
      set({ loading: false });
    }
  },
  ...
  getFriends: async () => {
    try {
      ...
      console.error("ERROR when getFriends", error);
      set({ friends: [] });
    } finally {
      set({ loading: false });
    }
