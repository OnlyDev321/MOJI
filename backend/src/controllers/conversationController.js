...
export const createConversation = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Group name and list nember is required" });
    }
    ...
    if (!conversation) {
      return res.status(400).json({ message: "Conversation is invalid" });
    }
    ...
    console.error("ERROR when make conversation", error);
...
export const markAsSeen = async (req, res) => {
  try {
    ...
    if (!last) {
      return res.status(200).json({ message: "not message to mark as seen" });
    }

    if (last.senderId.toString() === userId) {
      return res
        .status(200)
        .json({ message: "sender don't need mark as seen" });
    }
